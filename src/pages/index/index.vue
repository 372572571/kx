<template>
  <div>
    <bac-wx-service :show.sync="bacWxService.show"></bac-wx-service>
    <div class="index-head">
      ...
      <!---->
      <div style="height: 30px;margin-top: 30rpx">
        <image src="../../static/images/lamp.png" mode="aspectFit" style="height: 40rpx; width: 30rpx;display: inline-blocki; margin-left: 30rpx;vertical-align: top;" ></image>
        <span class="index-title" style="display: inline-block;margin-left: 20rpx">服务</span>
      </div>
      <bac-box-list @open="clickPush" ref="profile"></bac-box-list>
...
<script>
import {go} from '../../common/js/goto'
import BacBoxList from '@/components/bac-box-list/bac-box-list'
import BacWxService from '@/components/bac-wx-service/bac-wx-service'
...
export default {
  ...
  methods: {
    ...
    clickPush () {
      var there = this
      setTimeout(() => {
        there.$refs.profile.$data.newPromise.then((resolve) => {
          if (resolve.id === '0') {
            go(resolve.id)
            return
          }
          there.bacWxService.show = true
        }).catch((reject) => {
          console.log(reject)
        })
      }, 200)
